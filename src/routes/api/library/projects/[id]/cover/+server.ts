...
import { setProjectCoverAsset } from '$lib/server/library/organization';
...
export async function POST({ params, request }: { params: { id: string }; request: Request }) {
	...
	try {
		const project = setProjectCoverAsset(params.id, body.asset_id);
...
function errorMessage(error: unknown) {
	return error instanceof Error ? error.message : 'Project cover could not be updated';
}
