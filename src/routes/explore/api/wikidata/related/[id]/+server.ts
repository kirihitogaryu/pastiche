...
export async function GET({ params, url }: { params: { id: string }; url: URL }) {
	...
	try {
		...
		return json(
			{ error: errorMessage(error, 'Related Wikidata works failed to load') },
			{ status: 502 }
		);
	}
}

function parseRelatedParams(
	id: string,
	url: URL
): { id: string; limit: number; cursor?: string } | { error: string } {
	...
	if (cursor !== undefined && !/^\d+$/.test(cursor))
		return { error: 'Invalid related works cursor' };
