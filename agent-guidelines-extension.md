...
- **Capture panel UI** — use one page-local panel in Chrome and Firefox. Do not register a browser-native side panel/sidebar or popup. The content script owns only the lightweight host and drag target; the full Svelte capture workbench runs in an extension iframe.
- **No UI frameworks in content scripts** — content script code must be plain JS/TS with no framework dependencies. The framed capture workbench uses Svelte (matching the main Pastiche app). Do not introduce a different framework.
...
**1. The durable capture tray is the source of truth for selection state.**
The page's visual indicators (highlight ring, selected badge) and capture panel are views of the service-worker-owned tray, never independent state. If they disagree, the tray wins.
...
**3. Keep the injected panel bounded.**
The content script may inject the hover ring, selected badge, and one page-local capture host containing the extension's Svelte iframe. Do not duplicate the workbench in page DOM or open a second native browser surface.

**4. Capture mode must always be escapable.**
Pressing Escape must always cleanly exit capture mode, restore the cursor, and remove the overlay — on every page, including pages with aggressive event listeners. A stuck capture mode is a critical bug.
